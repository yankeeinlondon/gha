...
export class WorkflowGenerator {
  ...
  private async processWorkflow(sourceFile: string): Promise<string> {
    const type = this.config.type || 'workflow';
    
    // Resolve the template based on type
    let resolved: any;
    
    if (type === 'workflow') {
      resolved = await this.resolver.resolveWorkflow(
        sourceFile,
      ...
      // For jobs, steps, and actions, just load and parse without workflow-specific validation
      const fullPath = path.join(this.config.sourceDir, sourceFile);
      const content = await fs.promises.readFile(fullPath, 'utf8');
      resolved = yaml.load(content);
    }
  ...
  private async findWorkflowFiles(): Promise<string[]> {
    // Determine which directory to search based on type
    const type = this.config.type || 'workflow';
    ...
    const targetDir = path.join(this.config.sourceDir, searchDir);
