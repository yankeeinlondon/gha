...
import type { GitHubContext, EventPayload } from './types';
...
export function compareWorkflows(actual: any, expected: any): WorkflowComparison {
  ...
  const diff = diffLines(expectedYaml, actualYaml);
  const hasDifferences = diff.some((part: any) => part.added || part.removed);
